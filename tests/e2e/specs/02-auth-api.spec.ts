 ...
 * Login:    POST /v1/auth/token   → 200 (JSON body, not form)
 */
...
test.describe('Auth API — v1', () => {
  ...
  test('GET /v1/auth/me not available (not in openapi) — verify protected resource', async ({ request }) => {
    // Acquire token first
    const loginResp = await request.post(`${API}/v1/auth/token`, {
      data: { username: EMAIL, password: PASSWORD },
    });

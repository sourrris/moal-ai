...
FORMULAE=(python@3.11 postgresql@16)
MISSING=()
...
echo "Initializing PostgreSQL role/database..."
...
"$PIP" install -r "$ROOT_DIR/backend/requirements-dev.txt" >/dev/null

echo "Applying Alembic migrations..."
(cd "$ROOT_DIR/backend" && DATABASE_URL="postgresql+asyncpg://risk:risk@localhost:5432/risk_monitor" "$ROOT_DIR/.venv/bin/alembic" -c alembic.ini upgrade head)

...
- Publishes alert payloads to Redis channel (`risk.alerts.live`).
- Subscribes Redis Pub/Sub channel and broadcasts over WebSocket.
- Authenticated WebSocket endpoint (`/ws/alerts?token=...`).
- Health checks: `/health/live`, `/health/ready`.
